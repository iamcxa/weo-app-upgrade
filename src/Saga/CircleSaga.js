...
import { Circle, Handler } from "~/Api";
import Config from "~/Config";
import { Dialog, Fcm, Geolocation, Logger, Permission, t } from "~/Helper";
...
export function* fetchGetStayCircles({ onSuccess } = {}) {
  ...
  while (true) {
    ...
    console.log("isInternetReachable=>", next);
    console.log("navigationRef=>", navigationRef);
    console.log("navigationRef=>", navigationRef.getState());
    console.log("navigationRef=>", navigationRef.getCurrentRoute());

    if ((next !== isInternetReachable && next) || next) {
...
export function* fetchPutHomeCircle({ circle, onSuccess }) {
  yield put(AppStateActions["app/onLoading"](true, null, { hide: false }));
  const isInternetReachable = yield select(
    (state) => state.appState.netInfo.isInternetReachable
  );
...
export function* fetchGetCircleRanking({ onSuccess }) {
  // yield put(AppStateActions['app/onLoading'](true, null, { hide: false }));
  try {
    const apiToken = yield select((state) => state.user.apiToken);

    const geoData = yield call(Geolocation.getCurrentPosition);
    ...
    const { data: res } = yield call(
      ...
      Circle.getCircleRanking()
    );
    console.log("getCircleRanking res=>", res);
    if (res.success) {
      yield put(
        CircleActions.updateCircleStore({
          ranking: res.data,
        })
      );
      if (typeof onSuccess === "function") {
